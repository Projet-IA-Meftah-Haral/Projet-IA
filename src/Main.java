...
public class Main {
    public static void main(String[] args) throws Exception {
        System.out.println("Bienvenue sur le jeu du Quarto !");
        ...
        try (Scanner sc = new Scanner(System.in);) {
            ...
            while (!joueur1.a_gg() && !joueur2.a_gg()) {
                Plateau plateau = new Plateau();

                plateau.affichagePlateau();

                if (tour_joueur < 0.5) {
                    joueur2.deposerPiece(joueur1.choixPiece(sc), sc);

                    tour_joueur = 1;
                } else {
                    joueur1.deposerPiece(joueur2.choixPiece(sc), sc);
